...
const Handler = ({ data, variantId }) => {
    useEffect(()=>{},[data])
        ...
        const [isActive, setIsActive] = useState(data.priceBox.subscribe.hide ? 1 :2);
    if (!data) return null
    return (
        <div className={styles.accordionItem}>
           {!data.priceBox.oneTime.hide&& <><div className={`${styles.accordionTitle} { ${isActive == 1 && styles.accordionTitleActive} ${isActive == 1 && styles.p25_15}`} onClick={() => setIsActive(1)}>
            ...
            {!data.priceBox.subscribe.hide&& <><div className={`${styles.accordionTitle} ${isActive == 2 && styles.accordionTitleActive}`} onClick={() => setIsActive(2)}>
            ...
            {isActive == 2 && <div><PriceBox isActive={isActive} data={data} variantId={variantId}/>
            </div>}
            </>}
        </div>
    )
